...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2.5 group">
            ...
            <div>
              ...
              <span className="text-[10px] font-semibold tracking-wider text-slate-400 uppercase block -mt-0.5">
          ...
          <div className="hidden md:flex items-center gap-3">
            {isAuthenticated ? (
              ...
              <div className="flex items-center gap-2.5">
                <Link
                  to="/login"
                  className="text-sm font-semibold text-slate-600 hover:text-slate-900 px-3.5 py-1.5 rounded-lg hover:bg-slate-100 transition-colors"
                >
                  Sign In
                </Link>
                <Link
                  to="/scan"
                  className="inline-flex items-center gap-1.5 text-sm font-bold px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white shadow-xs transition-colors"
                >
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white px-4 pt-3 pb-5 space-y-1.5 shadow-md animate-in fade-in duration-150">
          {navLinks.map((link) => {
            if (link.authRequired && !isAuthenticated) return null;
            const Icon = link.icon;
            const isActive = location.pathname === link.path;
            ...
            {isAuthenticated ? (
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <Link
                  to="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="text-center text-sm font-bold py-2.5 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50"
                >
                  Sign In
                </Link>
                <Link
                  to="/register"
                  onClick={() => setMobileMenuOpen(false)}
                  className="text-center text-sm font-bold py-2.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700"
                >
                  Register
                </Link>
              </div>
